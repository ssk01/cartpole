...
DQN 论文（2015 Nature）的 Figure 4 用 t-SNE 展示了网络最后隐藏层的内部表征——把 128 维向量压缩到 2 维，看网络怎么"组织"它见过的游戏状态。我们在 CartPole 上做了同样的事：让训练好的模型玩 50 局，收集 25000 个状态，提取每个状态在最后隐藏层的 128 维表征，用 t-SNE 降到 2 维，然后用四种不同的颜色去"照亮"同一批点。

![DQN 内部表征的 t-SNE 可视化：按 Q 值、最优动作、杆角度、剩余寿命四种视角上色](./dqn_tsne_visualization.png)
*图：四张图的点位置完全相同（同一批 t-SNE 结果），只是颜色不同——相当于四个"滤镜"。左上按 Q 值上色，可见安全/危险状态自动分区；右上按动作上色，左推/右推形成两大块；左下按杆角度上色，和动作图高度对应（验证了"杆往哪倒就往哪推"）；右下按剩余寿命上色，快死的状态集中在边缘。*

核心发现和 DQN 论文一致：**网络不是按输入数值的相似度组织表征，而是按"这个状态值多少分"来组织**。感知不同但价值相似的状态被映射到邻近位置——网络学到的是因果结构，不是表面特征。
